...
func (g Geometria) getAreaEsfera(errList *[]error) float64 {
	...
	if err != nil || raio < 0 {
		err := fmt.Sprintf("Raio da esfera %s inválido, deve ser um número positivo e válido", g.Estrutura)
		*errList = append(*errList, errors.New(err))
		return 0
	}
...
func (g Geometria) getAreaCubo(errList *[]error) float64 {
	...
	if err != nil || lado < 0 {
		err := fmt.Sprintf("Lado do cubo '%s' é inválido: %s, deve ser um número positivo e válido", g.Estrutura, g.Lado)
		*errList = append(*errList, errors.New(err))
		return 0
	}
...
func (g Geometria) getAreaCilindro(errList *[]error) float64 {
	...
	if err != nil || altura < 0 {
		err := fmt.Sprintf("Altura do cilindro %s é inválida, deve ser um número positivo e válido", g.Estrutura)
		*errList = append(*errList, errors.New(err))
		return 0
	}
	...
	if err != nil || raioBase < 0 {
		err := fmt.Sprintf("Raio da base do cilindro %s é inválido, deve ser um número positivo e válido", g.Estrutura)
		*errList = append(*errList, errors.New(err))
		return 0
	}
...
func (g Geometria) getAreaParalelepipedo(errList *[]error) float64 {
	if g.Comprimento == "" || g.Largura == "" || g.Altura == "" {
		err := fmt.Sprintf("Comprimento, largura ou altura do paralelepipedo %s não foi informado", g.Estrutura)
		*errList = append(*errList, errors.New(err))
		return 0
	}
	...
	if err != nil || comprimento < 0 {
		err := fmt.Sprintf("Comprimento do paralelepipedo %s é inválido, deve ser um número positivo e válido", g.Estrutura)
		*errList = append(*errList, errors.New(err))
		return 0
	}
	...
	if err != nil || largura < 0 {
		err := fmt.Sprintf("Largura do paralelepipedo %s é inválida, deve ser um número positivo e válido", g.Estrutura)
		*errList = append(*errList, errors.New(err))
		return 0
	}
	...
	if err != nil || altura < 0 {
		err := fmt.Sprintf("Altura do paralelepipedo %s é inválida, deve ser um número positivo e válido", g.Estrutura)
		*errList = append(*errList, errors.New(err))
		return 0
	}
