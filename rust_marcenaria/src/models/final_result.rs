    ...
    pub fn from(movel: &Movel) -> Result<FinalResult, String> {
        ...
        match precos {
            Ok(precos_ok) => {
                Ok(
                    FinalResult {
                        tipo_movel: String::from(&movel.movel),
                        material: String::from(&movel.material),
                        preco_total: precos_ok.par_iter().map(|p| p.preco).sum(),
...
impl PrecoEstrutura {
    pub fn from(estruturas: &Vec<Geometria>, material: &str) -> Result<Vec<PrecoEstrutura>, String> {
        let madeira = Madeira::from_str(material).map_err(|_| {
            format!("{} não é um material válido, escolha entre Ébano, Carvalho e Pinho.", 
            material)
        })?;

        Ok(
            estruturas.par_iter().map(|estrutura| PrecoEstrutura {
